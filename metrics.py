...
def compute_roc(distances, matches, thresholds, fold_size=10):
    ...
    for fold_index, (training_indices, val_indices) \
        ...
        for threshold_index, threshold in enumerate(thresholds):
            ...
            predicts = predicts.byte()
            # print(predicts, val_matches)
            tp = torch.sum(predicts & val_matches).item()  # p->p
            fp = torch.sum(predicts & ~val_matches).item()  # n->p
            tn = torch.sum(~predicts & ~val_matches).item()  # n->n
            fn = torch.sum(~predicts & val_matches).item()  # p->n
