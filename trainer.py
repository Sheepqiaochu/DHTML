...
class Trainer(object):
    ...
    def train(self):
        for self.current_epoch in range(self.start_epoch, self.max_epoch + 1):
            ...
            if not (  # save models every five epochs
                    self.current_epoch % self.persist_stride):
                self.persist()
    ...
    def _get_matches(self, targets, logits, n=1):
        _, preds = logits.topk(n, dim=1)  # get the rank n's index of the line, which is the predicted class
